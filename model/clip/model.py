...
def make_activation_fake_quantize(
    dtype=torch.quint8,
...
class QATLinear(nn.Linear):
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        self.input_fake_quant = make_activation_fake_quantize(
            dtype=torch.quint8,
            qscheme=torch.per_tensor_affine,
        )
    ...
    @classmethod
    def from_float(cls, linear: nn.Linear) -> "QATLinear":
        r"""Create a qat module from a float module or qparams_dict
        Args:
            linear: an instance of nn.Linear
        Returns:
            An instance of QATLinear with observer and fake_quant modules inserted
        """
        module = cls(linear.in_features, linear.out_features, linear.bias is not None)
...
def build_model(
        state_dict: dict,
        h_resolution: int = None,
        w_resolution: int = None,
        vision_stride_size: int = None
):
    ...
    if vit:
        ...
        h_resolution = h_resolution or grid_size
        w_resolution = w_resolution or grid_size
        vision_stride_size = vision_stride_size or vision_patch_size
    else: #RN50
        ...
        h_resolution = h_resolution or output_width
        w_resolution = w_resolution or output_width
